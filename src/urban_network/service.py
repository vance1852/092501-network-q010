...
class NetworkService:
    def __init__(self,database=":memory:"): self.db=connect(database); self.auth=Auth(self.db); self.routes=RouteService(self.db,self.auth)
    def bootstrap(self):
        for uid,pwd,role in (("admin","network-admin","admin"),("operator","network-operator","operator"),("supervisor","network-supervisor","engineer")):
